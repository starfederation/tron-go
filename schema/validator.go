// Adapted from github.com/santhosh-tekuri/jsonschema/v6 for TRON integration.
package jsonschema
...
func (sch *Schema) validate(v any, regexpEngine RegexpEngine, meta *Schema, resources map[jsonPointer]*resource, assertVocabs bool, vocabularies map[string]*Vocabulary) error {
	...
	if _, err := vd.validate(); err != nil {
		...
		if _, ok := verr.ErrorKind.(*Group); ok {
			causes = verr.Causes
		} else {
		...
		return &ValidationError{
			...
			ErrorKind:        &SchemaError{Location: sch.Location},
			Causes:           causes,
		}
	}
...
func (vd *validator) validate() (*uneval, error) {
	...
	if s.Bool != nil {
		if *s.Bool {
			...
			return nil, vd.error(&FalseSchema{})
		}
	}

	// check cycle --
	if scp := vd.scp.checkCycle(); scp != nil {
		return nil, vd.error(&RefCycle{
			URL:              s.Location,
	...
	t := typeOf(v)
	if t == invalidType {
		return nil, vd.error(&InvalidJsonValue{Value: v})
	}
	...
	if s.Types != nil && !s.Types.IsEmpty() {
		...
		if !matched {
			return nil, vd.error(&Type{Got: t.String(), Want: s.Types.ToStrings()})
		}
	}
	...
	if s.Const != nil {
		...
		if k != nil {
			...
			return nil, vd.error(&Const{Got: v, Want: *s.Const})
		}
	}
	...
	if s.Enum != nil {
		...
		if !matched {
			return nil, vd.error(&EnumError{Got: v, Want: s.Enum.Values})
		}
	}
	...
	if s.Format != nil {
		...
		if err != nil {
			return nil, vd.error(&FormatError{Got: v, Want: s.Format.Name, Err: err})
		}
	}
...
func (vd *validator) objValidate(obj map[string]any) {
	...
	if s.MinProperties != nil {
		if len(obj) < *s.MinProperties {
			vd.addError(&MinProperties{Got: len(obj), Want: *s.MinProperties})
		}
	}
	...
	if s.MaxProperties != nil {
		if len(obj) > *s.MaxProperties {
			vd.addError(&MaxProperties{Got: len(obj), Want: *s.MaxProperties})
		}
	}
	...
	if len(s.Required) > 0 {
		if missing := vd.findMissing(obj, s.Required); missing != nil {
			vd.addError(&Required{Missing: missing})
		}
	}
		...
		if _, ok := obj[pname]; ok {
			...
			case []string:
				if missing := vd.findMissing(obj, dep); missing != nil {
					vd.addError(&Dependency{Prop: pname, Missing: missing})
				}
			case *Schema:
	...
	if len(additionalPros) > 0 {
		vd.addError(&AdditionalProperties{Properties: additionalPros})
	}
	...
	if s.PropertyNames != nil {
		for pname := range obj {
			...
			if err := sch.validate(pname, vd.regexpEngine, meta, resources, vd.assertVocabs, vd.vocabularies); err != nil {
				...
				verr.ErrorKind = &PropertyNames{Property: pname}
				vd.addErr(verr)
			}
		}
	}
	...
	for pname, reqd := range s.DependentRequired {
		if _, ok := obj[pname]; ok {
			if missing := vd.findMissing(obj, reqd); missing != nil {
				vd.addError(&DependentRequired{Prop: pname, Missing: missing})
			}
		}
	}
}
...
func (vd *validator) arrValidate(arr []any) {
	...
	if s.MinItems != nil {
		if len(arr) < *s.MinItems {
			vd.addError(&MinItems{Got: len(arr), Want: *s.MinItems})
		}
	}
	...
	if s.MaxItems != nil {
		if len(arr) > *s.MaxItems {
			vd.addError(&MaxItems{Got: len(arr), Want: *s.MaxItems})
		}
	}
	...
	if s.UniqueItems && len(arr) > 1 {
		...
		if k != nil {
			...
			vd.addError(&UniqueItems{Duplicates: [2]int{i, j}})
		}
	}
		...
		if s.AdditionalItems != nil {
			...
			case bool:
				if !additional && evaluated != len(arr) {
					vd.addError(&AdditionalItems{Count: len(arr) - evaluated})
				}
			case *Schema:
	...
	if s.Contains != nil {
		...
		if s.MinContains != nil {
			if len(matched) < *s.MinContains {
				vd.addErrors(errors, &MinContains{Got: matched, Want: *s.MinContains})
			}
		} else if len(matched) == 0 {
			vd.addErrors(errors, &Contains{})
		}

		// maxContains --
		if s.MaxContains != nil {
			if len(matched) > *s.MaxContains {
				vd.addError(&MaxContains{Got: matched, Want: *s.MaxContains})
			}
		}
	}
}
...
func (vd *validator) strValidate(str string) {
	...
	if s.MinLength != nil {
		if strLen < *s.MinLength {
			vd.addError(&MinLength{Got: strLen, Want: *s.MinLength})
		}
	}
	...
	if s.MaxLength != nil {
		if strLen > *s.MaxLength {
			vd.addError(&MaxLength{Got: strLen, Want: *s.MaxLength})
		}
	}
	...
	if s.Pattern != nil {
		if !s.Pattern.MatchString(str) {
			vd.addError(&Pattern{Got: str, Want: s.Pattern.String()})
		}
	}
	...
	if s.ContentEncoding != nil {
		...
		if err != nil {
			decoded = nil
			vd.addError(&ContentEncoding{Want: s.ContentEncoding.Name, Err: err})
		}
	}
...
func (vd *validator) numValidate(v any) {
	...
	if s.Minimum != nil && num().Cmp(s.Minimum) < 0 {
		vd.addError(&Minimum{Got: num(), Want: s.Minimum})
	}

	// maximum --
	if s.Maximum != nil && num().Cmp(s.Maximum) > 0 {
		vd.addError(&Maximum{Got: num(), Want: s.Maximum})
	}

	// exclusiveMinimum
	if s.ExclusiveMinimum != nil && num().Cmp(s.ExclusiveMinimum) <= 0 {
		vd.addError(&ExclusiveMinimum{Got: num(), Want: s.ExclusiveMinimum})
	}

	// exclusiveMaximum
	if s.ExclusiveMaximum != nil && num().Cmp(s.ExclusiveMaximum) >= 0 {
		vd.addError(&ExclusiveMaximum{Got: num(), Want: s.ExclusiveMaximum})
	}

	// multipleOf
	if s.MultipleOf != nil {
		if q := new(big.Rat).Quo(num(), s.MultipleOf); !q.IsInt() {
			vd.addError(&MultipleOf{Got: num(), Want: s.MultipleOf})
		}
	}
}
		...
		for i, sch := range s.OneOf {
			if err := vd.validateSelf(sch, "", matched != -1); err != nil {
				...
				if matched == -1 {
					...
					vd.addError(&OneOf{Subschemas: []int{matched, i}})
					break
				}
			}
		}
		if matched == -1 {
			vd.addErrors(errors, &OneOf{Subschemas: nil})
		}
	}
...
func (vd *validator) validateRef(sch *Schema, kw string) error {
	...
	if err != nil {
		refErr := vd.error(&Reference{Keyword: kw, URL: sch.Location})
		verr := err.(*ValidationError)
		if _, ok := verr.ErrorKind.(*Group); ok {
